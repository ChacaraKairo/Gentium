...
if (packageJson.version !== '1.1.0') {
  throw new Error(`Expected package.json version 1.1.0, received ${packageJson.version}.`);
}

if (appJson.expo?.version !== '1.1.0') {
  throw new Error(`Expected app.json version 1.1.0, received ${appJson.expo?.version}.`);
}

if (appJson.expo?.android?.versionCode !== 2) {
  throw new Error(`Expected Android versionCode 2, received ${appJson.expo?.android?.versionCode}.`);
}

if (appJson.expo?.ios?.buildNumber !== '2') {
  throw new Error(`Expected iOS buildNumber 2, received ${appJson.expo?.ios?.buildNumber}.`);
}

if (!fs.existsSync(releasePath)) {
  throw new Error('Expected docs/releases/v1.1.0.md.');
}
...
for (const key of [
  ...
  'bible.note',
  'bible.search.title',
  'bible.search.scopes.all',
  'bible.search.matchTypes.text',
  'bible.versions.title',
  ...
  'settings.localBackup.title',
  'settings.localBackup.action',
