...
if (packageJson.version !== '1.0.0') {
  throw new Error(`Expected package.json version 1.0.0, received ${packageJson.version}.`);
}

if (appJson.expo?.version !== '1.0.0') {
  throw new Error(`Expected app.json version 1.0.0, received ${appJson.expo?.version}.`);
}

if (appJson.expo?.android?.versionCode !== 1) {
  throw new Error(`Expected Android versionCode 1, received ${appJson.expo?.android?.versionCode}.`);
}

if (appJson.expo?.ios?.buildNumber !== '1') {
  throw new Error(`Expected iOS buildNumber 1, received ${appJson.expo?.ios?.buildNumber}.`);
}

if (!fs.existsSync(releasePath)) {
  throw new Error('Expected docs/releases/v1.0.0.md.');
}
